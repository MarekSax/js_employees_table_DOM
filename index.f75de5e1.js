...
for (const header of tableHeaders){
    ...
    if (header.textContent !== "Office") {
        ...
        const offices = [
            ...tableRows
        ].map((row)=>row.cells[2].innerText).filter((item, index, array)=>array.indexOf(item) === index).sort();
...
form.addEventListener("submit", (e)=>{
    ...
    if (fields.some((field)=>field.validity.valueMissing)) pushNotification("error", "All fields must be filled", "error");
...
tableCells.forEach((cell)=>{
    ...
    cell.addEventListener("dblclick", (e)=>{
        ...
        const handleCellInput = ()=>{
            ...
            if (target === target.parentElement.children[4]) target.innerText = `$${Number(cellInput.value).toLocaleString("en-US")}`;
        };
